...
import BestPlace from '../../../main/components/bestplaces/BestPlace';
...
describe('<BestPlace />', () => {
  ...
  it('should render <BestPlace /> with empty list of places', () => {
    const wrapper = shallow(<BestPlace />);
    expect(wrapper.state('places')).toEqual(expect.arrayContaining([]));
  });

  it('should calls componentDidMount after render <BestPlace />', () => {
